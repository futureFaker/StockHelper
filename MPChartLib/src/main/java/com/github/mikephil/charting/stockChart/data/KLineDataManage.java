...
public class KLineDataManage {
    ...
    private List<ILineDataSet> lineDataVolume = new ArrayList<>();
    ...
    public void parseKlineData(JSONObject object, String assetId, boolean landscape) {
        ...
        if (object != null) {
            ...
            if (data != null && data.length() > 10) {
                ...
                ArrayList<Entry> lineVolume5Entries = new ArrayList<>();
                ArrayList<Entry> lineVolume10Entries = new ArrayList<>();
                int len = data.length();
                for (int i = 0; i < len - 10; i++) {
                    KLineDataModel klineDatamodel = new KLineDataModel();
                    int index = len - i - 11;

                    klineDatamodel.setDateMills(DataUtils.getTimeStamp(data.optJSONArray(index).optString(0)));
                    klineDatamodel.setOpen(Double.parseDouble(data.optJSONArray(index).optString(1)));
                    klineDatamodel.setClose(Double.parseDouble(data.optJSONArray(index).optString(2)));
                    klineDatamodel.setHigh(Double.parseDouble(data.optJSONArray(index).optString(3)));
                    klineDatamodel.setLow(Double.parseDouble(data.optJSONArray(index).optString(4)));
                    klineDatamodel.setGain(Double.parseDouble(data.optJSONArray(index).optString(5)));
                    klineDatamodel.setHandTurnover(Double.parseDouble(data.optJSONArray(index).optString(6)));
                    klineDatamodel.setTotal(Double.parseDouble(data.optJSONArray(index).optString(7)));
                    klineDatamodel.setVolume(Double.parseDouble(data.optJSONArray(index).optString(8)));

                    double ma5 = (Double.parseDouble(data.optJSONArray(index).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 1).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 2).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 3).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 4).optString(2))) / 5.0;

                    klineDatamodel.setMa5(ma5);

                    double ma10 = (Double.parseDouble(data.optJSONArray(index).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 1).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 2).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 3).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 4).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 5).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 6).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 7).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 8).optString(2))
                            + Double.parseDouble(data.optJSONArray(index + 9).optString(2))) / 10.0;
                    klineDatamodel.setMa10(ma10);
                    klineDatamodel.setMa20(0);
                    klineDatamodel.setPreClose(Double.parseDouble(data.optJSONArray(index + 1).optString(2)));

                    double volume5 = (Double.parseDouble(data.optJSONArray(index).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 1).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 2).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 3).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 4).optString(8))) / 5.0;


                    double volume10 = (Double.parseDouble(data.optJSONArray(index).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 1).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 2).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 3).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 4).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 5).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 6).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 7).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 8).optString(8))
                            + Double.parseDouble(data.optJSONArray(index + 9).optString(8))) / 10.0;
                    klineDatamodel.setVolume5(volume5);
                    klineDatamodel.setVolume10(volume10);
                    ...
                    lineVolume5Entries.add(new Entry(i,i+offSet,(float) getKLineDatas().get(i).getVolume5()));
                    lineVolume10Entries.add(new Entry(i,i+offSet,(float) getKLineDatas().get(i).getVolume10()));
                }
                ...
                lineDataVolume.add(setALine(ColorType.blue,lineVolume5Entries,false));
                lineDataVolume.add(setALine(ColorType.yellow,lineVolume10Entries,false));
    ...
    public List<ILineDataSet> getLineDataVolume(){
        return lineDataVolume;
    }
